...
classdef flickerLightCrafter < handle
    properties %(Hidden)
        tcpConnection
    end
    methods
        ...
        %constructor
        function obj = flickerLightCrafter()
        ...
        function setDisplayModeHDMIVideoInput( obj, connection )
            header = obj.createHeader();
            header( 1 ) = uint8( hex2dec( '02' ) );	%packet type
            header( 2 ) = uint8( hex2dec( '01' ) ); %CMD1
            header( 3 ) = uint8( hex2dec( '01' ) ); %CMD2
            ...
            header( 5 ) = uint8( hex2dec( '01' ) ); %payloadLength LSB
            header( 6 ) = uint8( hex2dec( '00' ) ); %payloadLength MSB
            payload = uint8( hex2dec( '02' ) ); %payload
            packet = obj.appendChecksum( [ header; payload ] );
            %packet
            obj.sendData( packet, connection );
        end
        
        function setIPAdress( obj, connection, IP)
            header = obj.createHeader();
            header( 1 ) = uint8( hex2dec( '02' ) );	%packet type
            header( 2 ) = uint8( hex2dec( '08' ) ); %CMD1
            header( 3 ) = uint8( hex2dec( '00' ) ); %CMD2
            header( 4 ) = uint8( hex2dec( '00' ) ); %flags
            header( 5 ) = uint8( hex2dec( '04' ) ); %payloadLength LSB
            header( 6 ) = uint8( hex2dec( '00' ) ); %payloadLength MSB
            payload = uint8( [ IP(1); IP(2); IP(3); IP(4) ] ); %payload
            packet = obj.appendChecksum( [ header; payload ] );
            %packet
            obj.sendData( packet, connection );
        end
        
        function setVideoModeSetting( obj, connection, bitDepth, color)
            header = obj.createHeader();
            header( 1 ) = uint8( hex2dec( '02' ) );	%packet type
            header( 2 ) = uint8( hex2dec( '02' ) ); %CMD1
            header( 3 ) = uint8( hex2dec( '01' ) ); %CMD2
            header( 4 ) = uint8( hex2dec( '00' ) ); %flags
            header( 5 ) = uint8( hex2dec( '03' ) ); %payloadLength LSB
            header( 6 ) = uint8( hex2dec( '00' ) ); %payloadLength MSB
            payload = uint8( [ 60; bitDepth; color ] ); %payload
            packet = obj.appendChecksum( [ header; payload ] );
            %packet
            obj.sendData( packet, connection );
        end
        
        function setLEDcurrent( obj, connection, current)
            %current is a 1x3 array for RGB between 0 and 1024
            currentBytes = [round(mod(current,256)) floor(current/256) round(mod(current,256)) floor(current/256) round(mod(current,256)) floor(current/256)]';
            header = obj.createHeader();
            header( 1 ) = uint8( hex2dec( '02' ) );	%packet type
            header( 2 ) = uint8( hex2dec( '01' ) ); %CMD1
            ...
            header( 6 ) = uint8( hex2dec( '00' ) ); %payloadLength MSB
            payload = uint8(currentBytes); %payload
            packet = obj.appendChecksum( [ header; payload ] );
            %packet
            obj.sendData( packet, connection );
        end
